...
const StoryComponent: React.FC<StoryComponentProps> = ({ championId, role, tier, division }) => {
  const { getChampionImage, getChampionName, getRoleName, getTierDivisionName } = useResource();

  if (isChampionIdValid(championId)) {
    return <img src={getChampionImage(championId)} alt={getChampionName(championId)} />;
  }
  ...
  return <>{getTierDivisionName(tier, division)}</>;
};
